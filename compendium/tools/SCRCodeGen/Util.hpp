...
namespace codegen
{
  namespace util
  {
    ...
    public:
      ...
      JsonValueValidator(const Json::Value& data, std::string name, Json::ValueType type)
        : jsonName(std::move(name)),
          msg("Invalid value for the name '" + jsonName + "'. Expected ")
      {
        jsonVal = data[jsonName.c_str()];
        if (Json::Value::nullSingleton() == jsonVal)
        {
      ...
      JsonValueValidator(const Json::Value& data, std::string name, const ValidChoices<S>& choices)
        : jsonName(std::move(name)),
          msg("Invalid value for the name '" + jsonName + "'. Expected ")
      {
        ...
        if (std::none_of(choices.begin(), choices.end(), isCaseInsensitiveEqual))
        {
          ...
          std::copy(std::begin(choices), std::end(choices) - 1,
    ...
    namespace detail
    {
      ...
      inline void substitute(std::string& src, size_t index)
      {
      }
