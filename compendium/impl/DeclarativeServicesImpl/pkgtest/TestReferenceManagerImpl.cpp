  ...
  namespace scrimpl {
    ...
    TEST_P(ReferenceManagerImplTest, TestIsSatisfied)
    {
      ...
      // Register a service and check IsSatisfied is true
      {
        ...
        refManager.WaitForAsyncTask(); // wait until the async task is done.
    ...
    TEST_P(ReferenceManagerImplTest, TestEnque)
    {
      auto bc = GetFramework().GetBundleContext();
      auto fakeLogger = std::make_shared<FakeLogger>();
      auto fakeMetadata = GetParam();
      ...
      std::set<std::thread::id> taskThreads;
      size_t i = 0;

      std::function<std::thread::id(void)> func = [&refManager, &taskThreads, &i]() {
        std::packaged_task<void(void)> task([&taskThreads, &i]() {
          // record the id of the thread on which this task is performed
          taskThreads.insert(std::this_thread::get_id());
          i++;
        });
        refManager.Enqueue(std::move(task));
        return std::this_thread::get_id();
      };

      std::vector<std::thread::id> enqueCallerThreads = ConcurrentInvoke(func);
      refManager.WaitForAsyncTask(); // wait until queue is drained.

      std::vector<std::thread::id> thread_intersection;
      std::set_intersection(enqueCallerThreads.begin(), enqueCallerThreads.end(),
                            taskThreads.begin(), taskThreads.end(),
                            std::back_inserter(thread_intersection));
#if 0
      EXPECT_TRUE(thread_intersection.empty()) << "Tasks in the queue must run on threads other than the ones that enqueued those tasks";
#endif
      EXPECT_EQ(i, enqueCallerThreads.size()) << "i must be equal to the number of tasks submitted to the queue";
    }
