  ...
  namespace scrimpl
  {
    namespace util
    {
      ...
      private:
        ...
        template <typename T>
        static constexpr bool is_pertinent_container =
          std::is_same<T, std::vector<cppmicroservices::Any>>::value ||
          std::is_same<T, std::string>::value ||
          std::is_same<T, AnyMap>::value;

        // @brief Throws if the container T is of type @c Any or @c string or @c AnyMap and it's empty
        template <typename T>
        typename std::enable_if_t<is_pertinent_container<T>, void>
        ThrowIfEmpty(const T& value) const
        {
          if (value.empty())
          {
            std::string msg;
        ...
        template <typename T>
        static constexpr bool is_string_t = std::is_same<T, std::string>::value;

        template <typename TargetType>
        typename std::enable_if_t<is_string_t<TargetType>, void>
        ThrowIfValueAbsentInChoices(TargetType value, const std::vector<TargetType>& choices) const
        {
#if _WIN32
        ...
        template <typename TargetType>
        typename std::enable_if_t<!is_string_t<TargetType>, void>
        ThrowIfValueAbsentInChoices(const TargetType& targetval, const std::vector<TargetType>& choices) const
        {
        }
