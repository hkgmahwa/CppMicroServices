...
using cppmicroservices::scrimpl::util::AnyMap;
...
namespace {
  ...
  namespace ManifestHelper {
    ...
    static const AnyMap& GetTestManifest(const std::string& manifest_name)
    {
      ...
      try 
      {
        auto const& testMetadata = headers.AtCompoundKey("test_metadata." + manifest_name + "." + SERVICE_COMPONENT);
  ...
  TEST_F(MetadataParserImplV1Test, ParseValidManifest)
  {
    ...
    auto components = metadataparser->ParseAndGetComponentsMetadata(ManifestHelper::GetTestManifest("manifest_json"));
  ...
  {
    auto metadataparser = MetadataParserFactory::Create(1, GetLogger());
    auto components = metadataparser->ParseAndGetComponentsMetadata(ManifestHelper::GetTestManifest("manifest_dyn"));
    const auto component = components[0];
    ASSERT_EQ(component->activateMethodName, std::string("Activate"));
    ASSERT_EQ(component->deactivateMethodName, std::string("Deactivate"));
    ASSERT_EQ(component->modifiedMethodName, std::string("Modified"));
  ...
  {
    auto metadataparser = MetadataParserFactory::Create(1, GetLogger());
    auto components = metadataparser->ParseAndGetComponentsMetadata(ManifestHelper::GetTestManifest("manifest_mult_comp"));
  ...
  INSTANTIATE_TEST_CASE_P(FailureModes, InvalidMetadataTest,
    testing::Values(
      ...
      MetadataInvalidManifestState("manifest_illegal_ver2",
        "cppmicroservices::BadAnyCastException", /*isPartial=*/true),
      MetadataInvalidManifestState("manifest_illegal_ver3",
        "cppmicroservices::BadAnyCastException", /*isPartial=*/true),
      MetadataInvalidManifestState("manifest_illegal_comp",
        "Value for the name 'components' cannot be empty.", /*isPartial=*/true),
  ...
  INSTANTIATE_TEST_CASE_P(FailureModes, InvalidMetadataTestThroughLogger,
    testing::Values(
      ...
      MetadataInvalidManifestState("manifest_illegal_immediate",
        "Invalid value specified for the name 'immediate'. Could not load the component with index: 0"),
      MetadataInvalidManifestState("manifest_no_impl_class",
        "Missing key 'implementation-class' in the manifest. Could not load the component with index: 0"),
      MetadataInvalidManifestState("manifest_no_ref_name",
        "Missing key 'name' in the manifest. Could not load the component with index: 0"),
      MetadataInvalidManifestState("manifest_no_ref_interface",
        "Missing key 'interface' in the manifest. Could not load the component with index: 0"),
      ...
      MetadataInvalidManifestState("manifest_no_interfaces",
        "Missing key 'interfaces' in the manifest. Could not load the component with index: 0"),
      ...
      MetadataInvalidManifestState("manifest_empty_ref",
      ...
      MetadataInvalidManifestState("manifest_illegal_ref_interface",
        "Unexpected type for the name 'interface'. Exception: cppmicroservices::BadAnyCastException: "),
      MetadataInvalidManifestState("manifest_illegal_scope",
        "Invalid value 'global'. The valid choices are : [bundle, prototype, singleton]. Could not load the component with index: 0"),
      MetadataInvalidManifestState("manifest_illegal_ref",
        "Unexpected type for the name 'references'. Exception: cppmicroservices::BadAnyCastException"),
